...
A non-persistent Studio editing view that shows the whole original composition, the crop or padded area around it, and direct controls over its **Page Frame**. Its ordinary operation preserves Scene-to-physical scale while changing Page extent; its explicit fixed-page operation holds the physical Page and margins fixed while scaling or panning the frozen Scene. Ordinary preview shows the committed framed result edge-to-edge exactly as output will.
...
- Fixed-page `100%` is an absolute, centered contain-fit of the full frozen Composition at the locked drawable Page aspect. Aspect mismatch creates symmetric geometry-free padding; each later scale derives inversely from this stable reference rather than the prior draft, and preserves the current center so repeated changes do not compound drift.
- Fixed-page panning translates the same Page Frame and moves the stable center used by later scale changes. Returning the scale control to `100%` retains that panned center; Reset Frame is the only fixed-page action that recenters, and it restores the `100%` contain-fit without changing Page dimensions or margins.
