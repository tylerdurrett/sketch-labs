  ...
  it.each([
    ...
    (_name, aspect) => {
      ...
      for (const contour of layout.structuralContours) {
        contained(
          pointOnCircle(contour, contour.startAngle),
          contour.width / 2,
        );
...
describe("Scribble Moon source controls", () => {
  ...
  it("changes only the permission transition with Mask feather", () => {
    ...
    const justOutside = [
      sphere.center[0],
      sphere.center[1] - sphere.radius * 1.06,
    ] as const;
    const tonePoints = [
      sphere.center,
      justOutside,
    ...
    expect(hard.shadingMask.sample(justOutside)).toBe(0);
    expect(soft.shadingMask.sample(justOutside)).toBeGreaterThan(0);
    expect(soft.shadingMask.sample(justOutside)).toBeLessThan(1);
  });
});
...
describe("Scribble Moon field contract", () => {
  ...
  it("samples Composition Frame coordinates independently of diagnostic resolution", () => {
    const frame = resolveCompositionFrame(4 / 3);
    const point = [frame.width * 0.614, frame.height * 0.387] as const;
    const diagnosticResolutions = [64, 257, 1024];
    const samples = diagnosticResolutions.map(() =>
      samplePair(source({}, frame), point),
    );
