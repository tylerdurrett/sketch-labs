...
describe('baseline Scribble output golden', () => {
  ...
  it('preserves direct growth and shared RNG routing', () => {
    function directGrowth(model: ReturnType<typeof createScribbleModel>) {
      ...
      const nextRandomValue = rng.value()
      return { steps, nextRandomValue, rngCalls: { rangeCalls, valueCalls } }
    }

    const omitted = directGrowth(createScribbleModel(SOURCE, FRAME, CONTROLS))
    const explicitUndefined = directGrowth(
      createScribbleModel(SOURCE, FRAME, CONTROLS, undefined),
    )
