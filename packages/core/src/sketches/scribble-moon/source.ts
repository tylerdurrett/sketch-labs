 ...
 * affects only the Shading Mask's exterior permission transition.
 *
...
export interface ScribbleMoonControls {
  ...
  /** Normalized exterior permission feather, from hard `0` to broad `1`. */
...
export interface ScribbleMoonSource extends ToneSource {
  ...
  /** Resolved Composition Frame width of the mask's exterior transition. */
...
function permissionAt(
  ...
  permission = Math.max(
    permission,
    featheredPermission(haloSignedDistance, featherWidth),
  );
