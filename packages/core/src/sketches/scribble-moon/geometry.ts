...
/** Full permission inside a boundary, a smooth exterior feather, then exact zero. */
export function featheredPermission(
  signedDistance: number,
  featherWidth: number,
): number {
  if (signedDistance <= 0) return 1;
  if (featherWidth <= 0 || signedDistance >= featherWidth) return 0;
  return 1 - smoothstepUnit(signedDistance / featherWidth);
}
