...
const evidenceId = "issue-336-20260719T005436Z-responsive-review";
...
const observations = {
  schemaVersion: 1,
  review: "issue-336-product-studio-responsive-review",
  evidenceId,
  studioUrl,
  browser: {},
  policy: {
...
try {
  ...
  observations.video = {
    status: "not-recorded",
    reason:
      "The first host attempt's CDP-to-MediaRecorder bridge stalled before the first critical edit; the product review continued with timestamped screenshots and raw Worker/DOM timing logs.",
  };
  const seedEdit = await setNumericBeforeEnter(page, "#sketch-seed", reseed);
  const reseedIndex = seedEdit.workerIndex;
  await waitForProgress(page, reseedIndex);
  const staleCanvasBeforeProbes = await canvasHash(page);
  const staleBeforeProbes = await shadingSnapshot(page);
  await screenshot(page, "reseed-active-stale-retained");
  ...
  await screenshot(page, "active-progress-and-rolling-eta");
  ...
  const cancellationState = await page.evaluate(
    ...
    { workerIndex: reseedIndex, inputStart: chaosEdit.previousInputs },
  );
  ...
  await screenshot(page, "chaos-supersedes-reseed-latest-input-wins");
  ...
  await screenshot(page, "latest-chaos-result-settled");
  ...
  observations.transitions.supersedingEdit = {
    reseedWorkerIndex: reseedIndex,
  ...
  observations.transitions.reseedFixedScenario = {
    workerIndex: restoreChaos.workerIndex,
    action: restoreChaos.action,
    result: reseedFixedResult,
    shading: await shadingSnapshot(page),
  };

  const restoreSeed = await setNumericBeforeEnter(page, "#sketch-seed", primarySeed);
  await waitForProgress(page, restoreSeed.workerIndex);
  const fineEdit = await setNumericBeforeEnter(page, "#control-scribbleScale", 0.1);
  await waitForSuccess(page, fineEdit.workerIndex);
  await openShading(page, true);
  const fineCanvas = await canvasHash(page);
  const fineShading = await shadingSnapshot(page);
  await screenshot(page, "adopted-1m-budget-exhausted");
  const fineResult = await hashWorkerResult(page, fineEdit.workerIndex);
  observations.transitions.fineBudgetExhausted = {
    cancelledPrimaryRestoreWorker: workerSummary(
      await page.evaluate((index) => window.__ISSUE_336_REVIEW__.workers[index], restoreSeed.workerIndex),
    ),
    workerIndex: fineEdit.workerIndex,
    worker: workerSummary(await page.evaluate((index) => window.__ISSUE_336_REVIEW__.workers[index], fineEdit.workerIndex)),
  ...
  const activeWorker = workers[reseedIndex];
  ...
  observations.transitions.supersedingEdit.cancelledHeartbeat = {
  ...
  observations.assertions = {
    ...
    latestCanvasReplacedStale:
      latestCanvas.rgbaSha256 !== neatCanvas.rgbaSha256,
    reseedTargetExactlyStable:
      neatResult.targetHash === reseedFixedResult.targetHash,
    reseedIdentityChanged:
      neatResult.identityHash !== reseedFixedResult.identityHash,
    reseedSceneChanged:
      neatResult.sceneHash !== reseedFixedResult.sceneHash,
    fineTruthfullyBudgetExhausted:
      fineResult.diagnostics.termination === "budget-exhausted" &&
      fineShading.summary.includes("Budget exhausted") &&
      fineShading.detail.includes("bounded partial result, not a computation error"),
    fineMatchesAdoptedTarget: fineResult.targetHash === expectedFine.targetHash,
    fineMatchesAdoptedScene: fineResult.sceneHash === expectedFine.sceneHash,
    fineMatchesAdoptedResidual:
      fineResult.diagnostics.residualError === expectedFine.residualError,
  ...
  await writeFile(
    resolve(outputDirectory, "raw-browser-responsive-review.json"),
