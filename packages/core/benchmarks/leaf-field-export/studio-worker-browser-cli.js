...
if (!existsSync(puppeteerEntry)) {
  throw new Error('Set PUPPETEER_MODULE to the existing chrome-devtools Puppeteer entry')
}

const port = 4317
const url = `http://127.0.0.1:${port}/leaf-field-export-evidence.html`
const server = spawn(
  vite,
  [
    '--config',
    resolve(root, 'packages/core/benchmarks/leaf-field-export/studio-worker.vite.config.js'),
...
try {
  ...
  await page.waitForFunction(() => globalThis.__LEAF_FIELD_EXPORT_EVIDENCE__ !== undefined)
  const evidence = await page.evaluate(() =>
    globalThis.__LEAF_FIELD_EXPORT_EVIDENCE__.runAll(),
  )
...
async function waitForServer(target, child) {
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null) throw new Error(`Studio Vite server exited ${child.exitCode}`)
    try {
      const response = await fetch(target)
      if (response.ok) return
    } catch {
      // The deadline owns startup failure reporting.
    }
